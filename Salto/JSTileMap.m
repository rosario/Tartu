...
-(SKTexture*)textureForGid:(int)gid
{
  ...
  SKTexture* texture = self.textureCache[@(gid)];
	if(!texture)
	{
		...
		texture.filteringMode = SKTextureFilteringNearest; // RR
...
//	thinking to fix.
					...
					{
                        // RR. Aggiungo map.tileSize.height all Y
						pt = CGPointMake(x + (map.tileSize.width / 2.0), y + (map.tileSize.height / 2.0) + map.tileSize.height);
					}
//					SKTexture* texture = [tileset textureForGid:[num intValue] - tileset.firstGid + 1]; //RR fix below
					SKTexture* texture = [tileset textureForGid:[num intValue]];
					SKSpriteNode* sprite = [SKSpriteNode spriteNodeWithTexture:texture];
                    //sprite.anchorPoint = CGPointMake(0.5, -0.5); // RR non sono sicuro di questa aggiunta
					sprite.position = pt;
					sprite.zPosition = baseZPosition + ((map.zOrderCount - objectGroup.zOrderCount) * zOrderModifier);
					sprite.name = obj[@"name"];
					[map addChild:sprite];
#ifdef DEBUG
                    NSLog(@"Tileset first id: %d with gid: %d, name: %@", tileset.firstGid, [num intValue], sprite.name);
#endif
